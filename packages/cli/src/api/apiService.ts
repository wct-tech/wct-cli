...
process.env['OPENAI_API_KEY'] =
  process.env['OPENAI_API_KEY'] || process.env['THIRD_PARTY_API_KEY'];
process.env['WCT_API_TOOL_TIMEOUT'] =
  process.env['WCT_API_TOOL_TIMEOUT'] || '60000';
process.env['WCT_API_REQUEST_TIMEOUT'] =
  process.env['WCT_API_REQUEST_TIMEOUT'] || '300000';
...
> {
  return new Promise((resolve, reject) => {
    const toolNames = toolCallRequests
      .map((tool) => `${tool.name}: ${tool.args}`)
      .join(', ');
    const timeoutId = setTimeout(() => {
      console.warn(`工具 ${toolNames} 执行超时，已运行 ${timeoutMs}ms`);
      reject(new Error(`工具 ${toolNames} 执行超时，已运行 ${timeoutMs}ms`));
...
function formatError(
  errMessage: string,
  errDetail?: unknown,
): { error_message: string; error_detail: unknown } {
  return {
...
async function processGeminiStreamEvents(
  ...
  for await (const event of stream) {
    if ('value' in event) {
      console.log(
        `Session ${sessionId}: Gemini event:`,
        event.type,
        event.value,
      );
    } else {
...
async function handleCompletedTools(
  ...
  const completedAndReadyToSubmitTools = completedToolCalls.filter((tc) => {
...
function debugModelSelection(model: string, config: Config): void {
  ...
  console.log(
    `  3. 环境变量中的API URL: ${process.env['OPENAI_API_URL'] || '(未设置)'}`,
  );
  console.log(
    `  4. USE_OPENAI环境变量: ${process.env['USE_OPENAI'] || '(未设置)'}`,
  );
}
...
async function submitQuery(
  ...
  try {
    const prompt_id =
      config.getSessionId() + '########' + geminiClient.getHistory().length;
    ...
    if (toolCallRequests.length > 0) {
      console.log(
        `Session ${sessionId}: 调度 ${toolCallRequests.length} 个工具调用`,
      );

      // 使用超时处理函数执行工具
      const completedTools = await executeToolWithTimeout(
        scheduler,
        toolCallRequests,
        abortController,
      );
      ...
      if (toolResponse) {
        const finalStream = geminiClient.sendMessageStream(
          toolResponse,
          abortController.signal,
          prompt_id,
        );
        const finalResult = await processGeminiStreamEvents(
          finalStream,
          sessionId,
        );
...
async function streamGeminiToClient(
  ...
  try {
      ...
      while (true) {
        ...
        try {
          ...
          if (
            ...
            console.warn(
              `[streamGeminiToClient] Token计算错误，尝试不进行压缩: ${streamError.message}`,
            );
        ...
        console.log(
          `[streamGeminiToClient] 处理 ${toolCallRequests.length} 个工具调用`,
        );
        ...
        try {
          ...
          const completedAndReadyToSubmitTools = completedTools.filter((tc) => {
            const isTerminalState =
              tc.status === 'success' ||
              tc.status === 'error' ||
              tc.status === 'cancelled';
          ...
          const errorMessage =
            toolError instanceof Error ? toolError.message : '工具执行失败';
          const errorChunk = {
            id: `chatcmpl-${Date.now()}`,
            object: 'chat.completion.chunk',
            created: Math.floor(Date.now() / 1000),
    ...
    const errorMessage =
      initError instanceof Error ? initError.message : '服务初始化错误';
    if (!res.headersSent) {
      res.setHeader('Content-Type', 'text/event-stream');
      res.setHeader('Cache-Control', 'no-cache');
      res.setHeader('Connection', 'keep-alive');
...
async function checkApiConnection(apiKey?: string): Promise<boolean> {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.warn(
        `API服务器连接异常: ${response.status} ${response.statusText}`,
      );
      return false;
    }
  } catch (error) {
  ...
  (async () => {
            ...
            choices: [
              {
                index: 0,
                delta: {
                  error: formatError('请求处理超时', new Error('请求处理超时')),
                },
    ...
    try {
      ...
      if (!userMessage) {
        ...
        return res
          .status(400)
          .json(formatError('未提供用户消息', '未提供用户消息'));
      }
      ...
      if (stream) {
        ...
        try {
          ...
          const errorMessage =
            streamErr instanceof Error ? streamErr.message : '内部服务器错误';
        ...
        try {
          ...
          if (!res.headersSent) {
            return res
              .status(500)
              .json(formatError('非流式响应错误', errorMessage));
          } else if (!res.writableEnded) {
            res.end();
          }
        }
