...
function updateCoreDependencyVersion(version, packagePath) {
  try {
    ...
    if (
      ...
      packageJson.dependencies['@wct-cli/wct-cli-core']
    ) {
      packageJson.dependencies['@wct-cli/wct-cli-core'] = `${version}`;
      writeFileSync(packagePath, JSON.stringify(packageJson, null, 2) + '\n');
      console.error(
        `Updated @wct-cli/wct-cli-core dependency in ${packagePath} to ${version}`,
      );
    }
  } catch (error) {
    console.error(
      `Error updating core dependency in ${packagePath}: ${error.message}`,
    );
    process.exit(1);
  }
}
...
try {
  ...
  console.error(
    `Successfully published ${releaseVersion} to npm with tag ${npmTag}`,
  );
  process.exit(0);
} catch (error) {
